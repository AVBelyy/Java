 ...
 *
 * @author Anton Belyy, Georgiy Korneev
 *
 * @see info.kgeorgiy.java.advanced.implementor.Impler
 * @see java.lang.Class
 * @see javax.tools.JavaCompiler
 */
public class JarredImplementor implements Impler, JarImpler {
    /**
     * flag, true when JAR generation was requested
     * modified by either {@link ru.ifmo.ctddev.belyy.implementor.JarredImplementor#JarredImplementor()} or {@link #setGenerateJar(boolean)}
     */
    private boolean generateJar;

    /**
     * Constructs JarredImplementor, setting {@link #generateJar} flag to {@code false}.
     */
    public JarredImplementor() {
        generateJar = false;
    }
    ...
    public void implement(Class<?> clazz, File root) throws ImplerException {
        ...
        File outputFile = new File(root, clazz.getCanonicalName().replace(".", File.separator) + "Impl.java");
